    ...
    html += `
        <div class="data-section">
            <div class="data-item">
                ...
                <div class="data-value">
                    ${data.title ? `"${data.title}"` : 'Airbnb listing'} 
                    ${data.location ? `in ${data.location}` : ''}
                    ${data.rating ? `with ${data.rating}/5 rating` : ''}
    ...
    return `
        <div class="comprehensive-analysis">
            ...
            <div class="summary-box">
                <p>${analysis.summary || 'No summary available - check console for errors'}</p>
            </div>
...
async function getCurrentReviews() {
    try {
        ...
        // If not available, request from content script
        return new Promise((resolve) => {
            chrome.runtime.sendMessage({ type: 'extract_reviews' }, (response) => {
