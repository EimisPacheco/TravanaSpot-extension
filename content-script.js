  ...
  try {
    ...
    chrome.runtime.sendMessage(message, function(response) {
      if (chrome.runtime.lastError) {
        console.error('TravanaSpot: Message sending error:', chrome.runtime.lastError.message || chrome.runtime.lastError);
        // Common errors and their meanings
        if (chrome.runtime.lastError.message) {
          if (chrome.runtime.lastError.message.includes('Could not establish connection')) {
            console.error('TravanaSpot: Side panel or service worker not ready to receive messages');
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      async function collectReviewsWithScrolling() {
        ...
        // Final push: If we're between 50-99 reviews, try more aggressive techniques
        if (allReviews.length >= 50 && allReviews.length < 100) {
          console.log(`TravanaSpot: Close to 100 reviews (${allReviews.length}), making final aggressive push...`);
          ...
          // Final aggressive scrolling attempts
          for (let finalAttempt = 0; finalAttempt < 8 && allReviews.length < 100; finalAttempt++) {
            ...
            finalReviews.forEach(review => {
              ...
              if (!isDuplicate && review.name && review.text && allReviews.length < 100) {
                allReviews.push(review);
              }
            });
        ...
        try {
          const closeButtons = document.querySelectorAll(
            '[data-testid*="close"], [aria-label*="Close"], [aria-label*="close"], button[title*="close"], button[title*="Close"], .close-button'
          );
          for (const closeButton of closeButtons) {
            if (closeButton.offsetParent !== null && closeButton.getBoundingClientRect().width > 0) {
              closeButton.click();
              console.log('TravanaSpot: Closed modal/dialog');
              await new Promise(resolve => setTimeout(resolve, 1000));
              break;
            }
          }
        } catch (e) {
          console.log('TravanaSpot: Could not close modal:', e);
        }
        ...
        try {
          ...
          if (reviewElements.length > 0) {
            ...
            reviewElements.forEach((element, index) => {
              try {
                if (index >= 100) return; // Limit to first 100 reviews
                
                const review = {};
                
                // Extract reviewer name from h2 element
                const nameElement = element.querySelector('h2[elementtiming="LCP-target"]');
                ...
                // Extract review text
                const textElement = element.querySelector('.r1bctolv span');
                if (textElement && textElement.textContent) {
                  review.text = textElement.textContent.trim();
                }
                
                // Extract rating (count the star SVGs)
                const starElements = element.querySelectorAll('svg[aria-hidden="true"]');
